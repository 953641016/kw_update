# OpenClaw 核心模块迭代任务书：AI 关键词发布日期清洗管线升级

## 📌 任务指引
Hello OpenClaw，我是 Antigravity。
接下来的任务是将核心系统中负责校验产品发布日期的脚本架构升级为基于**“三级瀑布流 + 防大模型幻觉熔断机制”**的最新架构。

旧有架构因过多依赖传统的“搜索+全文阅读提取”，不仅导致高昂的 API 成本（~$0.05/次），并且在应对“不存在的爬虫污染产品”或“泄漏名称”时，极度容易产生 LLM 的时间编造幻觉。
新架构在我们的沙盒验证中抽样了数百个最高难度的混淆词条：
* **拦截率**：成功拦截了 100% 的虚假/泄漏产品（输出 NULL）。
* **精准率**：近期新品真实命中率 ~86%+。
* **降本率**：单次平均 API 调用成本压减至 `$0.007`，降幅超 85%。

你需要利用这份任务书，**将你工作区内负责查日期清洗的旧脚本替换为下方提供的新版脚本**，并进行回归验证。

---

## 🔍 架构原理与运行机制
了解这些机制有助于你更好地将该脚本接入你的宏观调度任务（Task Queue）中：

1. **第 ① 层：GitHub Release 极速探针 (省钱、零延迟)**
   通过 `api.github.com/search/repositories` 匹配。对开源模型有奇效，遇到符合星标和体积要求的 repo，优先提炼 Release date，没有的话取 repo 的 `created_at`。命中后直接退出流水线，彻底免去大模型开销。

2. **第 ② 层：Sonar Pro 定性判定与提取引擎 (核心枢纽)**
   调用 Perplexity `sonar-pro` API，系统为其设定了极度严苛的 System Prompt 控制输出。系统会强制要求其在返回值中不仅返回 date，还必须在搜不到时给出精准的枚举判定：
   - `not_exist` (判断为造词/污染词，直接触发智能熔断，切断流水线，输出 NULL)
   - `not_released` (仅仅是泄漏词或官宣未投入市场，同样触发熔断，输出 NULL)
   - `date_unknown` (产品合法且已上线，但 Sonar 在一次回复中未能精确确定日期。**此时允许触发下一级的兜底流**)

3. **第 ③ 层：Brave Search + GPT-4o 交叉验证 (终极兜底)**
   仅作为“营救手段”。若 Sonar 返回 `date_unknown`，则去 Brave Search 抓取网页并提取 Snippets（包含 URL 引用），并塞给纯度极高的 `gpt-4o` 进行交叉提取和日期容差校准（Diff <= 30天或更高要求），以补全一些偏门产品的发版日期。

---

## ⚙️ 环境变量与环境依赖
确保 OpenClaw 执行节点的工作区（Worker/Container）中配置了以下环境变量：
- `GITHUB_TOKEN`: 发起 Github 检索所需（个人鉴权 Token 即可）。
- `PERPLEXITY_API_KEY: 核心。用于 `sonar-pro`。
- `BRAVE_API_KEY`: 必须。兜底层检索网页所用。`: 核心。用于 `sonar-pro` 以及 `Perplexity Search API`。
- `OPENAI_API_KEY: 用于兜底提炼。
- `OPENAI_BASE_URL`: （可选）如果走代理，请填入反代地址，如 `https://api.openai.com/v1`。`: 用于调起兜底层的交叉验证大模型。
- （可选）`PRODUCTION=1`: 在脚本内可触发随机 1.5s ~ 3.0s 的 Jitter 防限流睡眠。

---

## 🛠️ 行动步骤 (Action Items)
对于主任务 Agent，请按照以下标准化动作迭代：
1. **备份旧版**：将现有的验证脚本重命名或执行本地仓库 Commit 归档。
2. **覆写测试代码**：将下方代码框中的所有内容拷贝覆盖至你工程目录里的 `verify_release_date.py`。
3. **干跑沙盒验证 (Dry-Run)**：
   分别抽样跑一下这两个词：
   - `DALL-E 4` (预期结果应快速命中并熔断拦截，不调用 openai，输出 `""`)
   - `Claude Opus 4.5` (预期结果应被 Sonar 或兜底捕获，输出精确的 `YYYY-MM-DD` 日期)
4. **集成上兵打仗**：确认功能完备后，将其重新并入你的 Feishu DB 定时扫描队列或清洗 Pipeline。

---

## 📄 完整的新版工业级脚本 (`verify_release_date.py`)
直接复制以下 Python 源代码。这是自带 SSL 重试、代理消除、以及格式泛化器的生产版本：
